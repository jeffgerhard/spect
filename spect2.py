...
Will look have these sections:

- go thru markdowns and compile their info into an instance of Spect class
- compare to any saved inventories in admin folder /// maybe i will keep this
    online too and add it to the admin info in the admin procedures? 
    this will identify any new or deleted files.
- also load any external dictionaries for archives areas and (eventually) 
    various non-blog pages of the overall website
- assemble tag and cat info
- if main is true: offer to build and modify pages via spect file routine
     - NEED TO REALLY CLEAN UP THE HTML ROUTINES

MAYBE -- jump directly to the possibility of publishing / uploading
       - build sitemap
       - generate RSS file(s)

LATER --- think about PREVIEWING pages in advance
        - integrating various ideas like spellcheck
...
import sitemap
from admin import sitevars, library, config
